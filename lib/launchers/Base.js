...
var BaseBrowser = function(id, emitter, captureTimeout, retryLimit, browserConfig) {
  ...
  this._getCommand = function() {
    var cmd = (browserConfig && browserConfig.cmd) || path.normalize(env[self.ENV_CMD] || self.DEFAULT_CMD[process.platform]);
  ...
  function browserConfigArgs(tmpDir, url) {
    return browserConfig &&
           browserConfig.args &&
           browserConfig.args.map(function(s) { return s.replace('$TEMPDIR', tmpDir).replace('$URL', url); }) ||
           [];
  }

  this._getOptions = function(url) {
    var opts = this._defaultOptions(url);
    var config = browserConfigArgs(this._tempDir, url);
    if (config.length) {
      opts = opts.concat(config);
    }
    return opts;
  };

  this._defaultOptions = function(url) {
    return [url];
  };
};
